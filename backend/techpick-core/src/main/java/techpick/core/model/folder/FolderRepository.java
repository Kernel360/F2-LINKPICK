...
public interface FolderRepository extends JpaRepository<Folder, Long> {
	...
	@Query("SELECT f FROM Folder f WHERE f.user.id = :userId AND f.folderType = techpick.core.model.folder.FolderType.UNCLASSIFIED")
	Folder findUnclassifiedByUserId(@Param("userId") Long userId);

	// TODO: QueryDSL 도입 후 리팩토링 필요
	@Query("SELECT f FROM Folder f WHERE f.user.id = :userId AND f.folderType = techpick.core.model.folder.FolderType.RECYCLE_BIN")
